// pages/new-select-seat/new-select-seat.js
...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      title: '选座',
      seatArea: getApp().globalData.screenHeight - getApp().globalData.statusBarHeight - (700 * getApp().globalData.screenWidth / 750),
  ...
  onShow: function () {
    ...
    requestUtil.requestPost(urlConfig.NEW_SEATS_URL, {
      "tel": wx.getStorageSync("tel"),
      "code": wx.getStorageSync("icode"),
      "planId": that.data.planId
    }, true).then((result) => {
      console.log("座位图", result)
      wx.hideLoading();
      if (result.errCode == 0) {
        let seatList = that.prosessSeatList(result);
        that.setData({
          movieName: result.data.movieName,
          planDetail: result.data.showTime,
          hallName: result.data.name,
          seatList: seatList,
          areaSeatTypeList: result.data.seatTypeList,
          areaSeatTypeListIsShow: result.data.seatTypeList == [] ? false : true,
          selectedSeat: [],
          totalPrice: 0,
          hidden: "hidden",
          price: parseInt(result.data.price),
          serviceFee: parseInt(result.data.serviceFee),
          isCheap: result.data.isCheap,
  ...
  prosessSeatList: function (response) {
    let resSeatList = response.data.seatList
    resSeatList.forEach(element => {
      ...
      // 加载座位的图标
      let seatType = response.data.seatTypeList;
  ...
  prosessMaxSeat: function (value) {
    ...
    //600 - 60 - 60
    let seatAreaHeight = this.data.seatArea - 120 * this.data.rpxToPx
  ...
  clickSeat: function (event) {
    ...
    for (const key in _selectedSeatList) {
      ...
      let serviceFee = parseInt(_selectedSeatList[key].serviceFee);
      let seatPrice = price;
  ...
  processUnSelected: function (index) {
    ...
    if (otherLoveSeatIndex !== null) {
      ...
      // temp.serviceFee = this.data.serviceFee
      // tempLove.price = this.data.price
      // tempLove.serviceFee = this.data.serviceFee
  ...
  createOrder: function () {
    ...
    requestUtil.requestPost(urlConfig.CREATE_ORDER_URL, {
      "planId": _this.planId,
      "tel": wx.getStorageSync("tel"),
      "code": wx.getStorageSync("icode"),
      "serviceFee": _this.serviceFee,
      "seatIdList": seatIds,
      "chPrice": _this.totalPrice,
    }).then((result) => {
  ...
  quickSeat: function (event) {
    ...
    // console.log("最佳座位组",bestSeatList)
